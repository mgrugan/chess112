...
def onAppStart(app):
    ...
    app.pieceValue = {'Q': 9, "R": 5, "B": 3, "N": 3, "P": 1, "K": 1000}
...
def makePromotion(app, mouseX, mouseY):
    ...
    for option in promotionOptions:
        ...
        if left <= mouseX <= left + optionWidth and top <= mouseY <= top + optionHeight:
            ...
            if checkmate(app.board, app.turn):
                app.gameOver = True 
            app.turn = not app.turn 
            app.selectedPiece = None
        counter += 1
...
def makesKingCheck(board, piece, targetRow, targetCol):
    ...
    ogRow, ogCol= piece.row, piece.col
...
def getComputerMove(app, board, isWhite):
    moves = []
    bestCapture = None
    bestValue = -1
    pieceValue = app.pieceValue

    for row in range(8):
        for col in range(8):
            piece = board[row][col]
            if piece and piece.isWhite ==isWhite:
                for move in piece.getLegalMoves(board):
                    ...
                    board[targetRow][targetCol] = piece
                    piece.row, piece.col = targetRow, targetCol
                    if not isKingInCheck(board,isWhite):
                        moves.append(((row, col), move))

                        if targetPiece:
                            targetValue = pieceValue.get(targetPiece.type, 0)
                            if targetValue > bestValue:
                                bestCapture = ((row, col), move)
    ...
    if bestCapture and bestCapture in moves:
        return bestCapture
    elif moves:
        return random.choice(moves)
    if checkmate(board, isWhite):
        app.gameOver = True
    else:
        return None
...
def computerMove(app):
    ...
    computerMove = getComputerMove(app, app.board, isWhite)
    if computerMove:
        (startRow, startCol), (endRow, endCol) = computerMove 
        piece = app.board[startRow][startCol]
        target = app.board[endRow][endCol] 

    
        app.board[startRow][startCol] = None
        app.board[endRow][endCol] = piece
        piece.row = endRow
        piece.col = endCol
